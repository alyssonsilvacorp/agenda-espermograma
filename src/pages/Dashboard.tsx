...
export default function Dashboard({ appointments, blockedDates, onNavigate }: DashboardProps) {
  ...
  const canceled = todayAppointments.filter(
    (appointment) => appointment.status === "Cancelado",
  ).length;
  ...
  const closedToday = blocked || weekend;

  const validScheduleTimesToday = closedToday
    ? []
    : SCHEDULE_TIMES.filter((time) => !isScheduleSlotExpired(today, time));

  const availableTimesToday = validScheduleTimesToday.filter(
    (time) =>
      !activeToday.some(
        (appointment) =>
  ...
  const totalSlotsToday = closedToday ? 0 : validScheduleTimesToday.length;
  const availableSlotsToday = closedToday ? 0 : availableTimesToday.length;

  const closedReason = blocked
  ...
  const cards = [
    {
      title: closedToday ? "Agenda fechada hoje" : "Vagas do dia",
      value: totalSlotsToday,
      helper: closedReason ?? formatLongDate(today),
      icon: closedToday ? CalendarOff : CalendarCheck,
    },
    {
      title: "Agendamentos",
      value: activeToday.length,
      helper: "Pacientes ativos hoje",
      icon: Users,
    },
    {
      title: "Disponíveis",
      value: availableSlotsToday,
      helper: closedToday ? "Agenda sem vagas disponíveis" : "Vagas restantes",
      icon: ClipboardList,
    },
    {
      title: "Confirmados",
      value: confirmed,
      helper: "Presenças confirmadas",
      icon: CheckCircle2,
    },
    {
      title: "Cancelados",
      value: canceled,
      helper: "Cancelamentos do dia",
      icon: XCircle,
    },
    {
  ...
  return (
    <div className="grid gap-6">
      ...
      {!blockedDateInfo && weekend && (
        <section className="rounded-md border border-amber-300 bg-amber-50 px-5 py-4 text-amber-900">
          <h3 className="text-lg font-semibold">Agenda indisponível hoje</h3>
          <p className="mt-1 text-sm">Sábado e domingo estão indisponíveis.</p>
        </section>
      )}

      {!closedToday && totalSlotsToday === 0 && (
        <section className="rounded-md border border-slate-200 bg-slate-50 px-5 py-4 text-slate-700">
          <h3 className="text-lg font-semibold">Horários encerrados</h3>
          <p className="mt-1 text-sm">
            Todos os horários de hoje já passaram. Não há vagas disponíveis para novos agendamentos.
          </p>
        </section>
      )}
                ...
                <strong className="text-2xl text-slate-950">
                  {
                    todayAppointments.filter(
                      (appointment) => appointment.examType === examType,
                    ).length
                  }
                </strong>
              </div>
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <span className="text-sm text-slate-500">
            {todayAppointments.length} registro(s)
          </span>
        </div>
        ...
        {todayAppointments.length === 0 ? (
          ...
          <div className="grid gap-2">
            {todayAppointments.map((appointment) => (
              <article
                key={appointment.id}
                className="grid gap-2 rounded-md border border-slate-100 p-3 sm:grid-cols-[0.45fr_1fr_1fr_auto] sm:items-center"
              >
